...
/**
 * use-user-excel-download.js
 * Client hook for downloading the users Excel report.
 * Mirrors use-user-pdf-download.js for Excel format.
 *
 * Responsibilities:
 * - Loading state management
 * - Fetch Excel from /api/users?format=xlsx with current filter params
...
const { EXCEL: L } = USER_CONFIG.UI.LABELS;

/**
 * Hook for downloading the users Excel report.
 * Supports two modes:
 ...
 */
export function useUserExcelDownload({ filters } = {}) {
  ...
  const doDownload = useCallback(async (userIds) => {
    ...
    try {
      ...
      const filename = filenameMatch?.[1] || "reporte_usuarios.xlsx";
